...
@section('content')
            ...
            <div class="col-12 mb-4 order-0">
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <button type="button" class="btn btn-sm btn-rounded btn-primary float-end mt-2" id="btn_refresh"><i
                                class='bx bx-refresh'></i></button>
                        <a href="{{ route('exportExcel') }}" class="btn btn-sm btn-rounded btn-primary float-end mt-2"
                            id="btn_refresh"><i class='bx bxs-download'></i></a>
                    </div>

                    <div class="card-body">

                        <div class="table-responsive">
                            <table class="table table-sm text-center" width="100%" id="table_berkas"
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="nibel" class="form-control mt-2" required
                                        placeholder="Masukan data NIBEL..">
                                </div>
                            </div>
                        </div>
    ...
    <form id="form_keterangan_berkas">
        @csrf
        <div class="modal fade" id="modal_keterangan" tabindex="-1" aria-labelledby="modal_keteranganLabel"
            aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.kirim', function() {
                ...
                if (confirm("Apakah anda yakin ingin mengirim berkas??") == true) {
                    ...
                    $.get('krimBerkas/' + jenis + '/' + history_id + '/' + berkas_id + '/' + proses_id,
                        function(data) {
                            reload();
            ...
            //kembali BT SU
            $(document).on('click', '.kembali_su', function() {

                if (confirm(
                        "Apakah anda yakin ingin mengembalikan berkas ke proses Pencarian dan Upload SU?"
                    ) == true) {
                    var berkas_id = $(this).attr('berkas_id');
                    // $('#berkas_id').val(berkas_id);
                    var jenis = $(this).attr('jenis');
                    ...
                    $('#btn_kembali' + history_id).attr('disabled', true);
                    $('#btn_kembali' + history_id).html(
                        '<span class="spinner-grow spinner-grow-sm" role="status" aria-hidden="true"></span><span class="visually-hidden">Loading...</span>'
                    );
                    $.get('kembaliSu/' + jenis + '/' + history_id + '/' + berkas_id + '/' + proses_id,
                        function(data) {
                            reload();
                            // $('#modal_loading').modal('hide');
                            Swal.fire({
                                ...
                                timer: 3000,
                                icon: 'success',
                                title: 'Berkas berhasil dikembalikan'
                            });
            ...
            $(document).on('click', '.kembali_bt', function() {

                if (confirm(
                        "Apakah anda yakin ingin mengembalikan berkas ke proses Pencarian dan Upload BT?"
                    ) == true) {
                    var berkas_id = $(this).attr('berkas_id');
                    // $('#berkas_id').val(berkas_id);
                    var jenis = $(this).attr('jenis');
                    ...
                    $.get('kembaliBt/' + jenis + '/' + history_id + '/' + berkas_id + '/' + proses_id,
                        function(data) {
                            reload();
                            // $('#modal_loading').modal('hide');
                            Swal.fire({
